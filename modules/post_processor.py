...
def _find_best_row_by_notes(group: pd.DataFrame) -> int:
    ...
    if "Заметки" not in group.columns:
        # Если колонки "Заметки" нет, берем первую строку
    ...
    for idx, row in group.iterrows():
        ...
        if pd.isna(notes) or notes == "":
            ...
            # Считаем количество символов как приблизительную меру "количества заметок"
            count = len(str(notes).strip())
    ...
    return notes_counts[0][1]  # Возвращаем индекс строки с наибольшим количеством
