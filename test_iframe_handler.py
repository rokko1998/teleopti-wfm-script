#!/usr/bin/env python3
"""
test_iframe_handler.py — Тестовый скрипт для проверки работы с iframe'ом
"""
...
# Константы
NEW_SITE_URL = (
    "http://t2ru-crmdb-03/Reports/report/%D0%9E%D1%82%D1%87%D0%B5%D1%82%D1%8B%20%D0%9A%D0%A6/%D0%9E%D1%82%D1%87%D0%B5%D1%82%20%D0%BF%D0%BE%20%D0%BF%D1%80%D0%B8%D1%87%D0%B8%D0%BD%D0%B0%D0%BC%20%D0%BE%D0%B1%D1%80%D0%B0%D1%89%D0%B5%D0%BD%D0%B8%D0%B9%20(%D1%81%D0%BE%20%D1%81%D1%86%D0%B5%D0%BD%D0%B0%D0%BC%D0%B8%20%D1%80%D0%B5%D0%B3%D0%B8%D1%81%D1%82%D0%B8%D0%B8)"
)
...
def setup_logging(level=logging.INFO):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('test_iframe_handler.log', encoding='utf-8')
        ]
    )
...
def main():
    ...
    logger.info("🚀 Запуск тестового скрипта для работы с iframe'ом")
    ...
    # Создаем WebDriver
    try:
        driver = get_driver(headless=False)  # Всегда запускаем с GUI для тестирования
        ...
        if open_new_site(driver, NEW_SITE_URL, logger):
            ...
            # Проверяем, что iframe режим включен
            logger.info(f"🔧 Iframe режим: {'включен' if report_handler.iframe_mode else 'выключен'}")

            # Ждем загрузки iframe
            logger.info("⏳ Ждем загрузки iframe...")
            import time
            time.sleep(5)
            ...
            try:
                ...
                logger.info(f"✅ Iframe найден: {iframe.get_attribute('src')}")
            except Exception as e:
                logger.error(f"❌ Iframe не найден: {e}")
                return 1

            # Ждем инструкций пользователя для тестирования
            logger.info("📊 Iframe найден, готов к тестированию")
    ...
    logger.info("✅ Тестовый скрипт завершен успешно")
    return 0
