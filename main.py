    ...
    try:
        ...
        if use_auto_date_processing:
            ...
            for idx, row in df_to_process.iterrows():
                ...
                import pandas as pd
                if pd.isna(row.get('Старт')) or pd.isna(row.get('Окончание')):
                    logger.warning(f"⚠️ Строка {mass_number}: обнаружены NaT значения в датах - пропускаем")
                    ...
                    try:
                        ...
                        if skip_reason == "already_processed":
                            ...
                            logger.info(f"✅ Нули записаны для {mass_number} (NaT в датах)")
                    ...
                    result = create_result_record(
                        mass_number,
                        'NaT_dates',  # специальное значение для строк с некорректными датами
                        0,  # lost_calls = 0
            ...
            for idx, row in df.iterrows():
                ...
                import pandas as pd
                if pd.isna(row.get('Старт')) or pd.isna(row.get('Окончание')):
                    logger.warning(f"⚠️ Строка {row['Номер массовой']}: обнаружены NaT значения в датах - пропускаем")
                    continue
